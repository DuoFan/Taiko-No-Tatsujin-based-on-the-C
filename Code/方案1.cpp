...
void Act_Note() {
	...
	for (int i = current_Note; i < Map[*Chosen].note_Number; i++) {
		if ((passed_Mileseconds >= Map[*Chosen].note[i].start - 793)
			&& Map[*Chosen].note[i].act)
			continue;
		else if (passed_Mileseconds >= Map[*Chosen].note[i].start - 793) {
...
void Note_Move() {
	...
	if (passed_Mileseconds >= station) {
		station = passed_Mileseconds + 7;
		for (int i = current_Note; i < Map[*Chosen].note_Number; i++) {
			if (Map[*Chosen].note[i].act) {
				Map[*Chosen].note[i].timing -= 7 * note_Speed;
...
void Update_Don() {
	...
	if (passed_Mileseconds - Don.next_Draw >= 500) {
		Don.next_Action += 2;
